...
- 用具体、日常的措辞替代编辑式简写、行政化表达和不必要的技术标签。将抽象的要求、限制和提醒落实为读者能够理解的具体行为，保留原有的必要性和约束程度。
- 保留让句子自然的助词、语气词、限定词和过渡词，把修饰关系和动作说完整。叙述完成情况时，使用完整、自然的动词表达，并按需要交代动作对象、去向和完成状态。允许为清楚、顺畅的表达适当增加字数，避免压缩成工作记录；用词随句意调整，删除相邻的同义重复，不机械补词或套用固定句式。
- 区分动作的目的、预期效果和已确认的实际成效。尚未验证效果时，表述意图或预期；有结果依据时，才表述已经取得的成效，程度应与证据相符。
- 加强或减弱确定性的措辞应有事实依据。调整语气时保留原有的确定性和效果程度，不为显得亲切、肯定或委婉而改变判断。提醒语用于确有必要引起注意的内容，不作为固定开场。
- 实用说明根据需要交代时间或条件、对象、动作、方法和目的。数量词或代词的所指不够明确时，补出对象名称。条件与动作之间使用自然的连接，说明可选操作时明确选择空间，保留必须执行事项的约束。密集的操作顺序拆成多个句子或步骤。
